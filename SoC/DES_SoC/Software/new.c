...
volatile int32 x_value;
volatile int32 y_value;
volatile int32 z_value;
volatile int32 temp_value;
...
void SysTick_ISR()
{
    ...
    temp_value = ((temp_h & 0x0F) << 8) | temp_l;
...
void write_upper_half_to_display(int8 value){
    // Write the value to the display, by writing to the pointers. This doesn't need to worry about encoding, as this is handled by the block
    pt2DISP->digit2 = value & 0xF; // write the lower nibble of the value to the third digit of the display
    pt2DISP->digit3 = (value >> 4) & 0xF; // write the upper nibble of the value to the fourth digit of the display
}

void main(){
    uint16 sw; // variable to store the value of the switches
    uint8 mode; // variable to store the mode of operation (tilt or position)
    static int32 x_pos = 0;
    static int32 y_pos = 0;
    static int32 z_pos = 0;
    static int32 x_vel = 0;
    static int32 y_vel = 0;
    static int32 z_vel = 0;
        ...
        if (mode == 0){ // This is tilt mode
            ...
            while(mode == 1) { // This is position mode, stay in this mode until the mode switch is toggled
                if (fresh_data_flag) { // check if new data is ready for processing
                    ...
                    LED_from_middle(x_value);
                    write_lower_half_to_display(y_value);
                    write_upper_half_to_display(z_value);
